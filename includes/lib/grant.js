...
let uiRouterGrant = angular.module('ui.router.grant', [uiRouter]);
uiRouterGrant
    ...
    .run(['$rootScope', '$state', '$log', 'GRANT_ERROR', function ($rootScope, $state, $log, GRANT_ERROR) {
        ...
        $transitions.onError({}, function (transition) {
            ...
            if (error && error.detail && error.detail.type === GRANT_ERROR) {
                /!*Prevent infinite loop if user has selected
                * a to state that is equal to current state
                * *!/
                if(transition.$to().name === transition.$from().name){
                    transition.abort();
                    $log.warn('grant: test "' + error.detail.test + '" stateTo redirect prevented. State "' + error.detail.stateTo + '" is the current state');
                    return;
                }
                /!*
                *
                * *!/
                if (!$state.get(error.detail.stateTo)) {
                    transition.abort();
                    $log.warn('grant: test' +
                        ' "' + error.detail.test + '" stateTo redirect prevented. State name "' + error.detail.stateTo + '" does not exist in ui-router $stateProvider');
                    return;
                }
                /!*Redirect to test fail state*!/
                $state.transitionTo(error.detail.stateTo);
            }
        });*/
...
export default uiRouterGrant.name;
